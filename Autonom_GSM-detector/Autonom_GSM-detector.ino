////////////////////////////////////////////////
// Актуально для плат на old bootloader
...
#define Chardge_min 3580          // Напряжение аккумулятора в мВ, ниже которого осущ-ся отправка смс по взведенному периоду пробуждения независимо от срабатывания датчика
#define critical_Chardge 3500     // напряжение, ниже которого отключается внешнее прерывание, модуль усыпляется на несколько дней, пока заряд не закончится

#define Level_IR  850     // Порог шума сигнала c  ИК-фоторезистора, за которым считать о наличии обнаружения
...
#include <GyverPower.h>               // библиотека режимов сна
...
String chardge;                         // переменная для хранения заряда аккума
...
void setup() {
  ...
  delay(5000);                           // физ.задержка, чтоб в случае подачи питания сразу и ардуино выключть его, а также при случайных перезагрузках успеть найти сеть

  sendATCommand(F("AT"), true);                       // проверка готовности модуля к работе и автонастройка скорости (ответом должен быть "ОК"),
  ...
  sendATCommand(F("AT+CNETLIGHT=0"), true);           // вырубаем светодиод индикатор
  ...
  //power.setSystemPrescaler(PRESCALER_2);          // частоту процессора устанавливаем на 8МГц (по умолчанию PRESCALER_1)
  ...
  digitalWriteFast(DTR_Pin,HIGH);                 // выключаем SIM800L (чтоб потом пробуждать каждый раз при заходе в бесконечный цикл)
...
void loop() {
  for(;;){
    ...
    if(flag_det == 1){                          // звонил номер из белого списка или пришло время детектирования
      chardge_parsing();                        // измеряем заряд аккумулятора
    ...
    else if(chardge.toInt() <= critical_Chardge){  // если напряжение критически низкое, то внешнее прерывание не включается
      sendSMS(innerPhone, chardge + "Low chardge. Sleep forever."); //отправляется сообщение с зарядом и фразой о том, что датчик дезактивируется
...
void detecting(){
    ...
    if (analogReadFast(analogPin) >= Level_IR){       // если сигнал с ИК-фоторезистора выше порога шума
      if (millis() - t_det > 500){                    // реагируем только на одно срабатывание за 500мс (чтоб дважды и более не реагировать на одно и то же событие)
        ...
        if(i >= 2 && (millis() - t_one) <= 10000){   // если было не менее 2 срабатываний в течении 10 секунд, то
          sendSMS(innerPhone, chardge +  ", 1");      // отправляем смс: [заряд аккум,  1]. Последнее -- знак того, что детектор срабатал
  ...
  if(i < 2 && (flag_true_call == 1 || chardge.toInt() <= Chardge_min)){   // в случае низкого уровня заряда или при звонке номера из белого списка отправляем смс с зарядом
    sendSMS(innerPhone, chardge);
  }
