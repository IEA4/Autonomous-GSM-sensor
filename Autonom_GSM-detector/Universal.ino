#define Charge_min 3150          // напряжение аккумулятора в мВ, ниже которого осущ-ся отправка смс по взведенному периоду пробуждения независимо от срабатывания датчика
#define critical_Charge 3350     // напряжение, ниже которого отключается внешнее прерывание, модуль усыпляется на несколько дней, пока напряжение не станет ниже допустимого
...
#define akkum_PIN  A1     // для измерения заряда аккумулятора
...
void setup() {
  ...
  pinMode(akkum_PIN, INPUT);            // для измерения заряда акумулятора
  analogReference(INTERNAL);            // источник опорного напряжения 1.1В
  analogRead(akkum_PIN);                // холостое считывание
...
String charge_parsing(){
  ...
  unsigned long ak_timer = millis();
  while (millis() - ak_timer < 500){
    voltage = analogRead(akkum_PIN);
  }
  return String(round(((float)voltage)*4.1));       // (умножение на 4.1, потому что напряжение отправляем в мВ)
}
...
void net_registration(){
  ...
  for (byte j = 0; j < 4; j++){
    ...
    if (j == 2 & flag_rst == 0){                 // если число запросов больше 2 и не было попытки перезагрузить gsm-модуль
  ...
  if(flag_rst == 1){                            //  если сеть была найдена после перезагрузки gsm-модуля // этот кусок кода применяется в loop
